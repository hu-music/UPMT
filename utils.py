...
def read_items(file_path): # other tracks 
    midi_obj = miditoolkit.midi.parser.MidiFile(file_path)
    # note
    DEFAULT_RESOLUTION = midi_obj.ticks_per_beat
    notes = []
    note_items=[]
    
    notes_all=[]
    for ins in range(len(midi_obj.instruments)):
        notes_all.append(len(midi_obj.instruments[ins].notes))
    print(notes_all)
    tracks=list(np.arange(len(notes_all)))
    del tracks[np.argmax(notes_all)]
    print('tracks that is not transferring:',tracks)
    for ins in tracks:
        notes += midi_obj.instruments[ins].notes
...
def read_items_single(file_path):
    midi_obj = miditoolkit.midi.parser.MidiFile(file_path)
    # note
    DEFAULT_RESOLUTION = midi_obj.ticks_per_beat
    notes = []
    ...
    for ins in range(len(midi_obj.instruments)):
        notes_all.append(len(midi_obj.instruments[ins].notes))
    #choose the track that has most note information
    print('tracks that is transferring:',np.argmax(notes_all))

    notes = midi_obj.instruments[np.argmax(notes_all)].notes
    notes.sort(key=lambda x: (x.start, x.pitch))
    for note in notes:
        note_items.append(Item(
            name='Note', 
...
def read_items_all(file_path):
    ...
    tempo_items = output
    return note_items, tempo_items    
def read_items1(file_path,file_path2):
    midi_obj = miditoolkit.midi.parser.MidiFile(file_path)
    midi_obj2 = miditoolkit.midi.parser.MidiFile(file_path2)
    # note
    DEFAULT_RESOLUTION = midi_obj.ticks_per_beat
    notes = []
    note_items=[]
    # for ins in range(len(midi_obj.instruments)):
    notes += midi_obj.instruments[0].notes
    for ins in range(len(midi_obj2.instruments)):
        notes += midi_obj2.instruments[ins].notes
        # notes += [a[i] for i in np.random.choice(len(a),int(len(a)/2),replace=False)]



    # notes += midi_obj3.instruments[0].notes
    # notes = midi_obj.instruments[0].notes
    notes.sort(key=lambda x: (x.start, x.pitch))
    for note in notes:
        note_items.append(Item(
    ...
    for tempo in midi_obj2.tempo_changes:
        tempo_items.append(Item(
            name='Tempo',
            start=tempo.time,
            end=None,
    ...
    # for tempo in midi_obj3.tempo_changes:
    #     tempo_items.append(Item(
    #         name='Tempo',
    #         start=tempo.time,
    #         end=None,
...
def write_midi(words, word2event, output_path, d = 480,prompt_path=None):
